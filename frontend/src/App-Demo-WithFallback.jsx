...
// Dati mock per il demo
const mockAnnouncements = [
  {
    id: 1,
    titolo: "Camera singola in centro storico",
    prezzo: 450,
    tipo_stanza: "Singola",
    citta: "Bologna",
    dimensione: 15,
    descrizione: "Bellissima camera singola nel cuore del centro storico di Bologna, a due passi dall'università. Luminosa e completamente arredata.",
  ...
  {
    id: 2,
    titolo: "Posto letto in camera doppia",
    prezzo: 280,
    tipo_stanza: "Doppia",
    citta: "Milano",
    dimensione: 20,
    descrizione: "Posto letto in camera doppia condivisa, vicino alla Bocconi. Ambiente gioviale e internazionale.",
    indirizzo: "Viale Bligny 10, Milano",
    immagini: [],
    telefono: "+39 347 987 6543",
    email: "affitti.milano@example.com"
  },
  {
    id: 3,
    titolo: "Monolocale arredato",
    prezzo: 650,
    tipo_stanza: "Monolocale",
    citta: "Roma",
    dimensione: 25,
    descrizione: "Grazioso monolocale completamente arredato nella zona universitaria di Roma. Ideale per studenti.",
    indirizzo: "Via del Castro Laurenziano 9, Roma",
    immagini: [],
    telefono: "+39 329 456 7890",
    email: "roma.student@example.com"
  }
];
...
const AnnouncementsListPage = () => {
  ...
  const fetchAnnouncements = async () => {
    try {
      ...
      // Prima prova a connettersi al backend
      ...
      setBackendStatus('connected');    } catch (err) {
      console.warn('Backend non disponibile, uso dati mock:', err);
      setBackendStatus('offline');
      
      // Usa dati mock se il backend non è disponibile
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{ color: 'white', fontSize: '18px', margin: 0 }}>
            {backendStatus === 'checking' ? 'Connessione al server...' : 'Caricamento dati demo...'}
          </p>
        </div>
      </div>
    );
    ...
    <div style={{
      ...
      {backendStatus === 'offline' && (
        <div style={{
          ...
          ⚠️ Modalità Demo - Backend offline. Visualizzazione dati di esempio.
          <button
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
            <div style={{
              background: backendStatus === 'connected' ? 'rgba(76, 175, 80, 0.2)' : 'rgba(255, 152, 0, 0.2)',
              padding: '8px 16px',
              borderRadius: '20px',
              fontSize: '12px',
              color: 'white',
              border: `1px solid ${backendStatus === 'connected' ? 'rgba(76, 175, 80, 0.5)' : 'rgba(255, 152, 0, 0.5)'}`
            }}>
              {backendStatus === 'connected' ? '🟢 Server Online' : '🟡 Modalità Demo'}
            </div>
        ...
        {announcements.length === 0 ? (
          <div style={{
            ...
            <p style={{ color: 'rgba(255, 255, 255, 0.8)', marginBottom: '30px' }}>
              {backendStatus === 'offline' ? 
                'Avvia il server backend per vedere gli annunci reali.' :
...
const SimpleAuthPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Se il backend non è disponibile, simula il login
      if (err.message.includes('fetch')) {
        setError('Backend offline - Simulazione login');
        setTimeout(() => {
          const mockUser = {
            id: 1,
            nome: formData.nome || 'Utente Demo',
    ...
    <div style={{
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit}>
          ...
          {error && (
            <div style={{
              background: error.includes('offline') ? 'rgba(255, 193, 7, 0.2)' : 'rgba(255, 107, 157, 0.2)',
              border: `1px solid ${error.includes('offline') ? 'rgba(255, 193, 7, 0.5)' : 'rgba(255, 107, 157, 0.5)'}`,
              color: 'white',
              padding: '10px',
              borderRadius: '10px',
              marginBottom: '15px',
...
// App principale
const App = () => {
  console.log("🎨 App UNI Home avviata con supporto offline!");
